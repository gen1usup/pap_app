...
/**
 * Streams active water break event.
 */
class ObserveActiveWaterBreakUseCase @Inject constructor(
    private val waterBreakRepository: WaterBreakRepository
) {
    operator fun invoke(userId: String): Flow<WaterBreakEvent?> =
        waterBreakRepository.observeActiveEvent(userId)
}
